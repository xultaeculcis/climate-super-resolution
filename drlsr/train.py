...
from pl_drln import DRLNLightningModule
...
def parse_args(arguments: argparse.Namespace = None) -> argparse.Namespace:
    ...
    parser = DRLNLightningModule.add_model_specific_args(parser)
    ...
    parser.add_argument('--experiment_name', type=str, default="drln-training")
...
) -> Tuple[DRLNLightningModule, SuperResolutionDataModule, pl.Trainer]:
    """
    Prepares everything for training. `DataModule` is prepared by setting up the train/val/test sets for specified fold.
    Creates new `DRLNLightningModule` Lightning Module together with `pl.Trainer`.
